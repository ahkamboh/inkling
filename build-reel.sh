...
# Usage: bash build-reel.sh <theme> [scene-set]
#   scene-set defaults to "reel" (the generic showreel). e.g. `bash build-reel.sh ink story`
set -e
cd "$(dirname "$0")"
THEME="${1:-ink}"
SET="${2:-reel}"
...
echo ">> rendering $SET frames ($THEME)"
node render-reel.js "$THEME" "$SET"

# per-scene transitions (declared in each scene); fall back to a default cycle
TRFILE="frames/$SET/$THEME/transitions.txt"
TR=()
if [ -f "$TRFILE" ]; then
...
SCENES=()
for d in $(ls -d frames/$SET/$THEME/s* | sort); do
  n=$(basename "$d" | tr -dc '0-9')
  ffmpeg -y -loglevel error -framerate $FPS -i "$d/%04d.png" -vf "$PAD" -c:v libx264 -crf 18 "out/r-$SET-$THEME-$n.mp4"
  SCENES+=("out/r-$SET-$THEME-$n.mp4")
done
...
fc="${fc%;}"
echo ">> stitching $N scenes (transitions: ${TR[*]})"
ffmpeg -y -loglevel error "${inputs[@]}" -filter_complex "$fc" -map "[v]" -r $FPS -c:v libx264 -crf 18 -pix_fmt yuv420p "examples/$SET-$THEME.mp4"

ffmpeg -y -loglevel error -i "examples/$SET-$THEME.mp4" -filter_complex "fps=12,scale=640:-1:flags=lanczos,split[a][b];[a]palettegen=stats_mode=diff[p];[b][p]paletteuse=dither=bayer:bayer_scale=2" "examples/$SET-$THEME.gif"
echo ">> done $SET-$THEME"
ls -la "examples/$SET-$THEME.mp4" "examples/$SET-$THEME.gif"
